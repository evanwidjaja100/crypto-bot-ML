...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--dataset", default=None, help="dataset dir; defaults to newest under data/datasets"
    )
    ...
    if "holdout" in splits and not splits["holdout"].empty:
        log.info("holdout rows=%d (never trained on)", len(splits["holdout"]))
    feature_cols = [c for c in splits["train"].columns if c.startswith("f_")]
    log.info(
        "dataset=%s rows=(train=%d val=%d test=%d) features=%d",
        Path(dataset_dir).name,
        *(len(splits[s]) for s in ("train", "val", "test")),
        len(feature_cols),
    )
    ...
    log.info(
        "logistic test: acc=%.4f f1=%.4f log_loss=%.4f",
        lr_te["accuracy"],
        lr_te["f1_macro"],
        lr_te["log_loss"],
    )
    ...
    log.info(
        "lgbm test: acc=%.4f f1=%.4f log_loss=%.4f",
        lgbm_te["accuracy"],
        lgbm_te["f1_macro"],
        lgbm_te["log_loss"],
    )
    ...
    def fit_predict(X_train, y_train, X_val, y_val):
        m = train_lgbm(
            X_train, y_train, X_val, y_val, settings.model.lgbm, seed=settings.model.seed
        )
        return m.predict(X_val), m.predict_proba(X_val)

    log.info(
        "walk-forward (%d folds, purge=%d)...",
        settings.model.walk_forward.n_splits,
        settings.labels.horizon,
    )
    wf = walk_forward(
        X_wf,
        y_wf,
    ...
    log.info(
        "walk-forward aggregate: %s",
        {k: round(v, 4) for k, v in wf["aggregate"].items() if k.startswith("mean_")},
    )
    ...
    log.info(
        "engine gate (test, real backtester): %s",
        {
            k: round(v, 4) if isinstance(v, float) else v
            for k, v in engine.items()
            if k in ("total_return", "profit_factor", "n_trades", "win_rate", "total_fees")
        },
    )
    ...
    base_meta = {
        ...
        "feature_set_id": fid,
        "label_set_id": meta.get("label_set_id", ""),
    ...
    save_model(
        logistic,
        {**base_meta, "model_id": model_id, "model_type": "logistic"},
        args.artifacts,
        framework="sklearn",
    )
    lgbm_meta = save_model(
        lgbm,
        {**base_meta, "model_id": model_id, "model_type": "lgbm"},
        args.artifacts,
        framework="lightgbm",
    )
    log.warning(
