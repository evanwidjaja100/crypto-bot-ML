...
def run_equivalence(
    ...
    *,
    ...
    if broker.direction != 0:
        last = df.iloc[-1]
        close_trade(
            broker.close_position(int(last["ts_ms"]), float(last["close"]), "end_of_backtest")
        )
...
def test_funding_two_boundaries_equivalent():
    ...
    n = 110
    o, h, low, c = flat_series(n)
    df = candles(n, o, h, low, c)  # bar 1 = boundary FUNDING_BOUNDARY, bar 97 = second boundary
...
def test_funding_does_not_leak_into_next_trade():
    ...
    fills, funding = b.enter_bar(
        pd.Series({"ts_ms": FUNDING_BOUNDARY, "open": 101, "high": 102, "low": 100, "close": 101})
    )
    assert fills == [] and funding == pytest.approx(-25.0 * 101.0 * 0.001)
    _, funding2 = b.enter_bar(
        pd.Series(
            {"ts_ms": FUNDING_BOUNDARY + IV, "open": 101, "high": 102, "low": 100, "close": 101}
        )
    )
    assert funding2 == 0.0
    fill1 = b.close_position(FUNDING_BOUNDARY + 2 * IV, 102.0, "signal_flat")

    b.open_position(FUNDING_BOUNDARY + 3 * IV, 102.0, 1, atr_value=1.0)
    assert b.enter_bar(
        pd.Series(
            {"ts_ms": FUNDING_BOUNDARY + 4 * IV, "open": 102, "high": 103, "low": 101, "close": 102}
        )
    ) == ([], 0.0)
...
def test_stop_loss_cooldown_equivalence():
    ...
    n = 25
    o, h, low, c = flat_series(n)
    low[5] = 96.0  # stop bar: low breaches the 98 stop
    df = candles(n, o, h, low, c, start_ms=1_700_000_000_000)  # non-boundary start
...
def test_take_profit_charges_maker_fee_equivalent():
    ...
    df = candles(n, o, h, low, c, start_ms=1_700_000_000_000)
