...
import pandas as pd
import requests
from test_runner import IV, START, FakeClient, FixedModel, make_settings
...
def _patch_main(tmp_path, monkeypatch, settings):
    ...
    monkeypatch.setattr(
        run_bot, "active_model", lambda artifacts: (FixedModel([0.2, 0.1, 0.7]), meta)
    )
    return df
...
def _main_args(tmp_path, *extra):
    return [
        ...
        str(tmp_path / "runner"),
        "--state-path",
        str(tmp_path / "runner" / "state.json"),
        *extra,
    ]


def test_once_with_failing_feed_reports_failure_exit_1(tmp_path, monkeypatch):
    """5.4: a --once run whose tick fails must exit non-zero, not 0."""
    ...
    assert run_bot.main(_main_args(tmp_path, "--once")) == 1


def test_once_with_healthy_feed_returns_zero(tmp_path, monkeypatch):
    """5.4: the happy path still exits 0."""
    settings = make_settings(tmp_path)
    df = _patch_main(tmp_path, monkeypatch, settings)
    monkeypatch.setattr(run_bot, "BybitClient", lambda testnet: FakeClient(df))
...
def test_stale_label_set_id_rejected(tmp_path, monkeypatch):
    """7.3: a model trained on different labels must not be served — the
    feature_set matches but the label_set does not -> exit 1."""
    settings = make_settings(tmp_path)
    df = make_long_frame()
    frame, cols = build_feature_frame(df, settings.features)
    fid = feature_set_id(settings.features.version, cols, settings.features.model_dump())
    meta = {
        ...
        "framework": "lgbm",
        "feature_set_id": fid,
        "label_set_id": "ffffffffffffffff",  # deliberately wrong
    }
    monkeypatch.setattr(run_bot, "load_settings", lambda: settings)
    monkeypatch.setattr(
        run_bot, "active_model", lambda artifacts: (FixedModel([0.2, 0.1, 0.7]), meta)
